...
float4 main(PixelShaderInput input) : SV_TARGET
{
	...
	{
		materialDiffuse = float4(0.4, 0.95, 0.25, 1.0);
		materialSpecular = float4(0.7, 0.6, 0.1, 1.0);
		texColour = float4(1.3, 0.2, 0.3, 1.0) * fractalNoise(input.posWorld.xz);
	}
	else
	{
		materialDiffuse = float4(1.6, 0.8, 1.4, 1.0);
		materialSpecular = float4(1.2, 0.8, 0.2, 1.0);
		texColour = float4(0.6, 1.0, 0.1, 1.0) * fractalNoise(input.posWorld.xz);
	}
	...
	if (diffuseFactor > 0)
	{
		specularFactor = pow(saturate(dot(viewDir, reflection)), 0.3 * 128);
	}
